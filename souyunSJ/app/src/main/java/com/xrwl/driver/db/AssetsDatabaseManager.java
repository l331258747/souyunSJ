 ...
 * 
 *       Using example: AssetsDatabaseManager.initManager(getApplication()); //
 *       this method is only need call one time AssetsDatabaseManager mg =
 *       AssetsDatabaseManager.getManager(); // get a AssetsDatabaseManager
 *       object SQLiteDatabase db1 = mg.getDatabase("db1.db"); // get
 *       SQLiteDatabase object, db1.db is a file in assets folder db1.??? //
 *       every operate by you want Of cause, you can use
 *       AssetsDatabaseManager.getManager().getDatabase("xx") to get a database
 *       when you need use a database
 */
public class AssetsDatabaseManager {
	private static String tag = "AssetsDatabase"; // for LogCat
	// private static String databasepath =
	// "/data/data/"+Constant.packageName+"/databases"; // %s is packageName
	...
	private String getDatabaseFilepath() {
		String path;
		String databasepath = context.getFilesDir().getPath();
		int pos = databasepath.lastIndexOf("/");
		if (pos > 0)
			path = databasepath.substring(0, pos) + "/databases";
		else
			path = databasepath + "/databases";
		return String.format(path, context.getApplicationInfo().packageName);
	}
